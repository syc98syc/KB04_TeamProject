...
//	@PostMapping("/jgig/searchWord")
//	public String insertSearchWord() {
//		return "search_word/searchWord";
//	}

	@PostMapping("/jgig/searchWordResult")
	public String searchWordHandler(PopularWordDto dto, @RequestParam("pw_word") String pw_word, Model model) {
		// pupulart_word 테이블에 insert
//		pw_word = pw_word.replaceAll("\\s+", "");
//		dto.setPw_word(pw_word);
//		dto.setPw_age(33 / 10 * 10); // 10단위만 넣기 
...
//		searchWordMapper.insert(dto);
		...
		try {
			...
			urlBuilder.append(
					"&" + URLEncoder.encode("pageNo", "UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /* 숫자로 관리 */
			urlBuilder.append(
					"&" + URLEncoder.encode("numOfRows", "UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /* 숫자로 관리 */
...
//	            System.out.println(map);
    ...
    public List<HashMap<String, String>> getResultMap(String data) throws Exception {
    	...
    	// totalCount
    	NodeList nodeList2 = (NodeList) xpath.compile("/response/body/totalCount").evaluate(document, XPathConstants.NODESET);
